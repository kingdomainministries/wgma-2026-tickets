...
/* ==================================================================
   3. EMAIL — swap EMAIL_USER / EMAIL_PASSWORD in .env to change the
      sending address. Gmail needs an App Password.
   ================================================================== */
...
async function mailTicket(order) {
  ...
  try {
    ...
    attachments.push({ filename: `wgma-ticket-${order.reference}.jpg`, content: png, cid: 'ticket' });
    hero = `<div style="margin:0 0 24px">
      <img src="cid:ticket" width="468" alt="${esc(tier.name)} ticket ${esc(order.reference)}"
  ...
  attachments.push({ filename: `wgma-qr-${order.reference}.png`, content: qr, cid: 'qr' });
  ...
  const html = shell(`
    ...
    <div style="text-align:center;margin:26px 0">
      <img src="cid:qr" width="180" alt="Ticket code ${esc(order.reference)}" style="background:#fff;padding:10px">
  ...
  await mailer.sendMail({
    from: `"Western Gospel Music Awards" <${process.env.EMAIL_USER}>`,
    to: order.email,
...
async function mailOrganiser(order) {
  await mailer.sendMail({
    from: `"WGMA Tickets" <${process.env.EMAIL_USER}>`,
    to: process.env.ORGANISER_EMAIL || process.env.EMAIL_USER,
...
app.listen(PORT, () => {
  ...
  const missing = ['EMAIL_USER', 'EMAIL_PASSWORD', 'ADMIN_KEY', 'DOMAIN']
  ...
  if (process.env.EMAIL_USER && process.env.EMAIL_PASSWORD) {
    mailer.verify()
      ...
      .catch(e => console.warn('Email NOT ready —', e.message));
  }
});
